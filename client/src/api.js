...
export const api = {
  async getTransactions(year, month) {
    const url = year && month
      ? `${BASE}/transactions?year=${year}&month=${month}`
      : `${BASE}/transactions`
  ...
  async deleteTransaction(id) {
    ...
    if (!r.ok) throw new Error('Error eliminando transacción')
    return r.json()
  },

  async getBudgets() {
    const r = await fetch(`${BASE}/budgets`)
    if (!r.ok) throw new Error('Error cargando presupuestos')
    return r.json()
  },
  ...
  async updateBudget(cat, amount) {
    ...
    if (!r.ok) throw new Error('Error actualizando presupuesto')
    return r.json()
  },
}
