...
export default function SplitView({ transactions }) {
  ...
  const paula  = expenses.filter(t => t.person === 'paula').reduce((a, t) => a + Number(t.amount_eur), 0)
  const agueda = expenses.filter(t => t.person === 'agueda').reduce((a, t) => a + Number(t.amount_eur), 0)
  const ambas  = expenses.filter(t => t.person === 'ambas').reduce((a, t) => a + Number(t.amount_eur), 0)
  ...
  return (
    <div style={{ background: '#fff', border: '0.5px solid #D3D1C7', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '1.25rem' }}>
      ...
      {diff <= 0.5 && total > 0 && (
        <div style={{ background: '#EAF3DE', borderRadius: 9, padding: '0.6rem 1rem', fontSize: 13, color: '#3B6D11' }}>¡Estáis al 50/50 este mes! 🎉</div>
      )}
      <div style={{ fontSize: 11, color: '#B4B2A9', marginTop: 8, textAlign: 'center' }}>Los gastos compartidos se dividen a partes iguales</div>
    </div>
  )
}
