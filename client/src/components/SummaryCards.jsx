...
export default function SummaryCards({ transactions }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: '1.25rem' }}>
      <Card label="💰 Ingresos"  value={fmt(income)}  color="#3B6D11" />
      <Card label="💸 Gastos"    value={fmt(expense)} color="#993556" />
      <Card label="⚖️ Balance"   value={(balance >= 0 ? '+' : '-') + fmt(balance)} color={balance >= 0 ? '#3B6D11' : '#993556'} />
    </div>
  )
}
