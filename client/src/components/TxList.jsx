...
function EditModal({ tx, onSave, onClose }) {
  ...
  const s = { ...inp, width: '100%', marginBottom: 8 }

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: '1.25rem', width: 340, border: '0.5px solid #D3D1C7' }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: '0.85rem' }}>✏️ Editar movimiento</h3>
        ...
        <input style={s} placeholder="Nota (opcional)" value={form.notes} onChange={e => set('notes', e.target.value)} />
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '8px', borderRadius: 9, border: '0.5px solid #D3D1C7', background: '#F8F7F6', cursor: 'pointer', fontSize: 13 }}>Cancelar</button>
          <button onClick={() => onSave(form)} style={{ flex: 1, padding: '8px', borderRadius: 9, border: 'none', background: '#D4537E', color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>Guardar</button>
        </div>
      </div>
    </div>
  )
...
export default function TxList({ transactions, onDelete, onEdit, filters, onFilterChange }) {
  ...
  const SortBtn = ({ col, label }) => (
    <button onClick={() => toggleSort(col)} style={{
      ...
      borderRadius: 6, padding: '3px 8px', fontSize: 11, cursor: 'pointer', color: sortBy === col ? '#993556' : '#5F5E5A',
    }}>
    ...
    <div>
      ...
      <div style={{ background: '#fff', border: '0.5px solid #D3D1C7', borderRadius: 12, padding: '0.75rem 1rem', marginBottom: 10 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
          <input style={inp} placeholder="🔍 Buscar..." value={filters.search || ''} onChange={e => onFilterChange({ ...filters, search: e.target.value })} />
          <select style={inp} value={filters.cat || ''} onChange={e => onFilterChange({ ...filters, cat: e.target.value })}>
            <option value="">Todas las categorías</option>
            {CATS.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
          </select>
          <select style={inp} value={filters.person || ''} onChange={e => onFilterChange({ ...filters, person: e.target.value })}>
      ...
      {sorted.length === 0 && <p style={{ fontSize: 13, color: '#888780', textAlign: 'center', padding: '1rem' }}>Sin movimientos</p>}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {sorted.map(t => {
          ...
          return (
            <div key={t.id} style={{ background: '#fff', border: '0.5px solid #D3D1C7', borderRadius: 10, padding: '0.6rem 1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: isIncome ? '#3B6D11' : '#993556' }}>{isIncome ? '+' : '-'}€{Number(t.amount).toFixed(0)}</div>
                    <div style={{ fontSize: 11, color: '#888780' }}>{dateStr}</div>
                  </div>
                  <button onClick={() => setEditing(t)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#D3D1C7' }}
                    onMouseEnter={e => e.target.style.color = '#D4537E'} onMouseLeave={e => e.target.style.color = '#D3D1C7'}>✏️</button>
                  <button onClick={() => onDelete(t.id)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#D3D1C7' }}
