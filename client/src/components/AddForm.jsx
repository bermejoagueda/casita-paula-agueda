...
const inp = {
  width: '100%', border: '0.5px solid #D3D1C7', borderRadius: 9,
  padding: '8px 10px', fontSize: 13, background: '#F8F7F6', color: '#2C2C2A', outline: 'none',
}

export default function AddForm({ onAdd }) {
  const [form, setForm] = useState({ desc: '', amount: '', cat: '', type: 'expense', date: today, person: 'ambas', notes: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ desc: '', amount: '', cat: '', type: 'expense', date: today, person: 'ambas', notes: '' })
    ...
    <div style={{ background: '#fff', border: '0.5px solid #D3D1C7', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '1.25rem' }}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          ...
          <input style={inp} type="number" placeholder="Importe (€)" min="0" step="0.01" value={form.amount} onChange={e => set('amount', e.target.value)} />
        </div>
        ...
        <button type="submit" disabled={loading} style={{
          ...
          fontWeight: 500, fontSize: 13, cursor: loading ? 'not-allowed' : 'pointer',
        }}>
