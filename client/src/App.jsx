...
export default function App() {
  ...
  const handleAddRecurring    = async (data) => { await api.addRecurring(data); await loadData() }
  const handleUpdateRecurring = async (id, data) => { await api.updateRecurring(id, data); await loadData() }
  const handleDeleteRecurring = async (id) => { await api.deleteRecurring(id); setRecurring(prev => prev.filter(r => r.id !== id)) }
  const handleGenerateRecurring = async () => { const res = await api.generateRecurring(year, month + 1); await loadData(); return res }

  const tabStyle = (t) => ({
    padding: '6px 11px', borderRadius: 8, border: tab === t ? 'none' : '0.5px solid #D3D1C7',
