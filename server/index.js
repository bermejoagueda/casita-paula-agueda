...
async function initDB() {
  ...
  if (txCount === 0) {
    ...
    await pool.query(`
      INSERT INTO transactions (description, amount, cat, type, date, person, notes) VALUES
...
app.get('/api/savings/:year/:month', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT * FROM savings WHERE year=$1 AND month=$2', [year, month]
    );
    res.json(rows[0] || { year, month, goal: 0, saved: 0 });
  } catch (err) {
    res.status(500).json({ error: 'Error' });
  }
});
...
app.put('/api/savings/:year/:month', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO savings (year,month,goal,saved) VALUES ($1,$2,$3,$4)
       ON CONFLICT (year,month) DO UPDATE SET goal=$3, saved=$4 RETURNING *`,
      [year, month, goal, saved]
    );
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: 'Error' });
